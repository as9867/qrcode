    ...
    $.fn.extend({
        ...
        qrcdr: function(options) {
            ...
            return this.each(function() {
                ...
                //save QR code image for darg
                function getPngFile(svgFile) {
                    var DOMURL = window.URL || window.webkitURL || window;
                    var svg = new Blob([svgFile], {type: 'image/svg+xml'});
                   ...
                   // canva.setHeight(window.innerHeight/2);
                   // canva.setWidth(380);


                   document.getElementById('lnkDownload').style.display="block";
                   document.getElementById('lnkDownload2').style.display="block";
                   document.getElementById('lnkDownload3').style.display="block";
                   ...
                   newImg.onload = function() {
                        ...
                        const imgSrc = $("#draggable-img-container > img").attr('src');
                        const imgPathArray = imgSrc.split('/');
                        $.ajax({
                            type: "POST",
                            url: relative + "ajax/png.php",
                            cache: false,
                            data: {imgdata: dataURL, filename: "preview.png",isDraggable:true,existingFile:imgPathArray[1]}
                        })
                        .fail(function(error) {
                        ...
                        .done(function(msg) {
                            if (msg == 'error') {
                                bootbox.alert({
                                 ...
                                 // alert(msg)
                               //  $("#draggable-img-container").html("");
                               //  $("#draggable-img-container").html("<img src='"+msg+"' ondragstart='onDragStart(event);' id='image' style='display: none;'/>");
                               ...
                               //  canvas.add(imgInstance);
                                ...
                                // var width1 =$("#draggable-img-container > img").attr('width'); 
                                // var height1=$("#draggable-img-container > img").attr('height');
                                // alert(width1) 
                                // alert(height1) 
                               ...
                               // if(!!activeObject){
                               //  // alert(msg)
                               // activeObject.setSrc('http://localhost/qrcode/'+msg, function(){canvas.renderAll();})
                               // }
                               // else{
                                // alert('2')
                               fabric.Image.fromURL(msg, function(myImg) {
                              ...
                              //  activeObject.setElement(myImg);

                              //   activeObject.set({width: width, height: height});
                                var img1 = myImg.set({ left: 100, top: 100 ,width:100,height:100});
                              canvas.add(img1); 
                               ...
                               // }
                                // $("#draggable-img-container > img").attr('src',msg).attr('class','example-draggable').attr('draggable',true) 
                            }
                            drawww();
                            // var object = canvas.getActiveObject();
                            // console.warn(object)
